...
| 2021 *Luminaria* code-presence check, **before any constant scan** | `libil2cpp.so` returns **0** AArch64 `ret` in **73,309,724** bytes and **83** THUMB `bx lr` in 60,130,336 against a chance rate of **458.76** -- below chance -- while `libunity.so` from the same package, ABI and toolchain returns **58,853** and **8,557**; the denominator for the game's own code is zero, [android-talesofluminaria-doc](https://github.com/vs-sr-dev/android-talesofluminaria-doc) |
| 2021 *Luminaria* constant scan, **AArch64, the first 64-bit ARM target** | **0** `movz` of 4078 or 4079 in any of six libraries, over **46,151 + 76,814 + …** move-wide and **112,485 + 507,008 + …** add/sub immediates; the only move-wide hits on any of the five are two `movz w8, #4080` in `libunity.so` -- and on this machine all five constants fit one `movz`, so one pass is a complete search |
| 2021 *Luminaria* structural probe, **six ARM32 images** | **0 candidate sites** in all six, and **0** `orr rX, rX, #0xFF00` in five of the six out of 114 `#0xFF00` immediates -- with the note that the probe has never been calibrated on a true ARM positive printed above the numbers |
| 2021 *Luminaria* blind sweep, candidate count | **217** offsets pass `plausible()` in the 1995 dialect and **53,689** in the 1997 one over 299,304,225 bytes -- one per 1,379,282 and one per 5,575 -- which is why the flat pass does not finish and why the candidate count is quoted as the denominator |
| 2020 *Crestoria* against 2021 *Luminaria*, **two Android gacha, fourteen months apart** | **46** files in common and all 46 Google Play Services boilerplate; **2** internal names in common out of **6,994** against **1,333**, and they are `MainScene` and `Node` |
